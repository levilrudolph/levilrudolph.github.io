...
(() => {
  ...
  const MAX_DIM = 2000;
  // Never store below this on the long side, whatever the slot's rendered
  // width — a slot can be re-framed (zoomed) or grow with the layout later.
  const MIN_DIM = 1600;
  ...
  async function toDataUrl(file, targetW) {
    ...
    try {
      const cap = Math.min(MAX_DIM, Math.max(MIN_DIM, Math.round(targetW * 2) || MIN_DIM));
